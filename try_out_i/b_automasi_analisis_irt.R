...
for (i in subtes_tps){
  ...
  #df_profil_tes_1PL <- data.frame(coef(profil_tes_1PL, simplify = TRUE))
  df_profil_tes_2PL <- data.frame(coef(profil_tes_2PL, simplify = TRUE))

  #print(df_profil_tes_3PL)

  #guessing_parameter <- data.frame(subset(df_profil_tes_3PL, select = c(3)))
  #parameter_1PL <- data.frame(subset(df_profil_tes_1PL, select = c(2)))
  
  parameter_2PL_1 <-data.frame(subset(df_profil_tes_2PL, select = c(1))) # parameter diskriminasi
  #parameter_2PL_2 <-data.frame(subset(df_profil_tes_2PL, select = c(2)))
  ...
  # Menyeleksi soal berdasarkan parameter diskriminasi
  data_soal <- c()
  
  for (j in 1:nrow(parameter_2PL_1)){
    if (parameter_2PL_1[j,] < 0.65)
      data_soal <- c(data_soal, j)
  }
  ...
  # Menyiapkan data yang digunakan dalam analisis IRT 1PL, 2PL
  if (!(is.null(data_soal))){
    data_mentah <- data_mentah[, -c(data_soal)]
  }

  print(paste("Analisis", i, "menggunakan model 1PL & 2PL"))
  ...
  uji_anova <- anova(profil_tes_1PL, profil_tes_2PL)
